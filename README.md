...
- два Gemini API key: «Актёр» для основного разговора и «Режиссёр» для
  advisory-анализа;
- отдельная вкладка «Эффекты» с десятью независимо включаемыми и полностью
  настраиваемыми разговорными/DSP-эффектами;
- все эффекты по умолчанию выключены, поэтому существующие роботы продолжают
  использовать стабильный медиатракт 1.1.0.
...
## Стабильные точки и изменения

Неприкосновенные точки отката `v1.0.0-stable` и `v1.1.0-stable` описаны в
`STABLE_BASELINE.md`. Изменения находятся в `CHANGELOG_1.1.0.md` и
`CHANGELOG_1.2.0.md`.
