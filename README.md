...
3. Только после этого backend запускает выбранный на карточке робота телефонный
   маршрут:
   - `API LPTracker` — прежний `POST /lead/{lead_id}/call`, при котором
     LPTracker соединяет Asterisk с клиентом;
   - `Напрямую` — Asterisk сам набирает первый телефон лида через
     `PJSIP/lptracker-endpoint`, а после ответа подключает существующий
     `chan_websocket`-медиаконтур.
4. Оба маршрута используют одну подготовленную Gemini-сессию и одну логику
   диалога. Переключатель меняет только способ установления соединения.
...
- переключатель способа звонка `API LPTracker` / `Напрямую` на каждой карточке;
...
- отдельная вкладка эффектов с локальным фильтром коротких реплик и регулируемой отсрочкой перебивания;
